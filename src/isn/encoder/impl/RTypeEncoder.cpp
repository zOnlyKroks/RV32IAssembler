#include "RTypeEncoder.h"

uint32_t RTypeEncoder::encode(const std::string& mnemonic,
                              const std::vector<std::string>& operands,
                              uint32_t address,
                              const std::map<std::string, uint32_t>& labels) {
    if (operands.size() != 3) {
        throw AssemblyException("Invalid operands for " + mnemonic);
    }

    const int rd = parseRegister(operands[0]);
    const int rs1 = parseRegister(operands[1]);
    const int rs2 = parseRegister(operands[2]);
